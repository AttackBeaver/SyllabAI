...
def _generate_with_github(topic: str, lesson_type: str, temperature: float) -> str:
    ...
    prompt = (
        f"Ты — ассистент преподавателя вуза/колледжа. Напиши краткое содержание "
        f"о том, что должно содержаться в занятии типа '{lesson_type}' по теме '{topic}'. "
        f"Укажи ключевые термины и знания, которые должны получить студенты. "
        f"Если это практическая работа, указывай какие работы должны выполнить студенты в рамках темы практической. "
